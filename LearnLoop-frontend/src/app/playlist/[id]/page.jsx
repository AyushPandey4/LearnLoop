...
export default function PlaylistPage() {
  ...
  const hasRewatchVideos =
    playlist?.videos.some((video) => video.status === "rewatch") || false;
  ...
  const fetchPlaylist = async () => {
    try {
      ...
      if (err.response?.status === 401 || err.response?.status === 403) {
        setError(
          "You are not authorized to view this playlist. Please log in again."
        );
      } else {
  ...
  const updateVideoStatus = async (videoId, newStatus) => {
    try {
      await axios.patch(`${API_URL}/api/video/${videoId}/status`, {
        status: newStatus,
      });
  ...
  const generateAiSummary = async (videoId) => {
    try {
      const response = await axios.post(
        `${API_URL}/api/video/${videoId}/generate-summary`
      );
  ...
  const copySummaryToNote = async (videoId) => {
    try {
      const response = await axios.post(
        `${API_URL}/api/video/${videoId}/summary-to-note`
      );
      ...
      setPlaylist((prev) => {
        ...
        return {
          ...
          videos: prev.videos.map((video) =>
            video.id === videoId
              ? { ...video, notes: response.data.notes }
              : video
          ),
        };
      });
  ...
  const markAllAsComplete = async () => {
    ...
    const videosToUpdate = playlist.videos.filter(
    ...
    try {
      const updatePromises = videosToUpdate.map((video) =>
        axios.patch(`${API_URL}/api/video/${video.id}/status`, {
          status: "completed",
        })
      );
  ...
  const handleTogglePin = async (videoId) => {
    try {
      ...
      const response = await axios.patch(
        `${API_URL}/api/playlist/${id}/toggle-pin-video/${videoId}`
      );
  ...
  const handleReorderVideos = async (videoPositions) => {
    try {
      ...
      await axios.patch(`${API_URL}/api/playlist/${id}/reorder`, {
        videoPositions,
      });
    } catch (err) {
            ...
            <div className="flex gap-4 p-1 bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 w-fit">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-gray-700/50 px-2 py-0.5 rounded-full text-xs">
                    {
                      playlist.videos.filter((v) => v.status === "rewatch")
                        .length
                    }
                  </span>
                </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-300">
                        Videos are displayed in the original order set by the
                        playlist creator. The numbers in the blue circles
                        indicate each video's position.
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-indigo-900/20 backdrop-blur-sm border border-indigo-500/20 rounded-xl p-6 transform hover:scale-[1.01] transition-all duration-300">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-300">
                          This is a custom playlist. You can drag and drop
                          videos to customize their order.
                        </p>
                      </div>
                    </div>
                  </div>
