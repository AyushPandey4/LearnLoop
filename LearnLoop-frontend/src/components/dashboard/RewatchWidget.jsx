...
export default function RewatchWidget() {
      ...
      for (const playlist of playlists) {
        try {
          const playlistResponse = await axios.get(
            `${API_URL}/api/playlist/${playlist.id}`
          );
  ...
  return (
    <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg shadow-md overflow-hidden mb-8">
      {/* Header */}
      <div
        className="px-4 py-3 bg-purple-100 dark:bg-purple-900/40 flex justify-between items-center cursor-pointer"
          ...
          {loading ? (
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {rewatchVideos.map((video) => (
                <div
                  key={video.id}
                  className="flex items-center bg-white dark:bg-gray-800 rounded-md shadow-sm overflow-hidden"
                >
