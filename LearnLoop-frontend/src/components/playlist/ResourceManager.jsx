...
export default function ResourceManager({
  ...
  const getResourceIcon = (type) => {
    const resourceType =
      resourceTypes.find((t) => t.value === type) || resourceTypes[4]; // Default to "other"
  ...
  const handleAddResource = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/video/${videoId}/resources`,
        {
  ...
  const handleDeleteResource = async (resourceId) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${API_URL}/api/video/${videoId}/resources/${resourceId}`
      );
    ...
    <div className="mt-4">
      ...
      {resources && resources.length > 0 ? (
            ...
            >
              <div className="flex items-center min-w-0">
                <span className="flex-shrink-0 w-5 h-5 flex items-center justify-center mr-2">
                  {typeof getResourceIcon(resource.type) === "string" &&
                  getResourceIcon(resource.type).startsWith("http") ? (
                    <img
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400 italic">
          No resources added yet. Add links to related resources like GitHub
          repos, documentation, or notes.
        </div>
      )}
    </div>
  );
