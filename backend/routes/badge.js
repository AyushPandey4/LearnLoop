...
const checkStreakBadges = async (userId) => {
  try {
    // Soon will add logic to track daily streak based on user daily activity
...
// Evaluate user progress & assign badges
router.post("/check-badges", authenticateToken, async (req, res) => {
  try {
    ...
    const playlistCompletionBadges = await checkPlaylistCompletionBadges(
      req.user.id
    );

    badgesAdded =
      firstPlaylistBadge ||
      videoMilestoneBadges ||
      streakBadges ||
      playlistCompletionBadges;
...
// Show all possible badges (for frontend use)
...
// Remove duplicate playlist completion badges
router.post("/cleanup-duplicates", authenticateToken, async (req, res) => {
  try {
    ...
    const titlesWithDuplicates = Object.keys(badgesByTitle).filter(
      (title) =>
        badgesByTitle[title].length > 1 && title.startsWith("Completed: ")
    );
...
// Synchronize badges with playlists - ensuring consistency
router.post("/sync", authenticateToken, async (req, res) => {
  try {
    ...
    const playlistBadges = badges.filter((badge) =>
    ...
    const completedPlaylists = playlists.filter(
      (playlist) => playlist.completed
    );
    const existingBadgeTitles = new Set(
      playlistBadges.map((badge) => badge.title)
    );
