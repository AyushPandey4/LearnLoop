...
// Creates a new playlist from YouTube URL or custom content
router.post("/add", authenticateToken, async (req, res) => {
  try {
    ...
    if (
      ...
      !user.categories.includes(finalCategory)
    ) {
...
router.get("/", authenticateToken, async (req, res) => {
  try {
    ...
    const playlists = await Playlist.find({ userId: req.user.id }).select(
      "name category videos completed createdAt ytInfo ytPlaylistUrl ytPlaylistId isCustomPlaylist"
    );
    ...
    const allVideos = await Video.find({
      ...
      "playlistId status timeSpent notes thumbnail viewCount duration title ytId position"
    );
    ...
    const playlistsWithProgress = playlists.map((playlist) => {
      ...
      const inProgressVideos = playlistVideos.filter(
        (video) => video.status === "in-progress"
      ).length;
      const rewatchCount = playlistVideos.filter(
        (video) => video.status === "rewatch"
      ).length;
      const totalTimeSpent = playlistVideos.reduce(
        (sum, video) => sum + (video.timeSpent || 0),
        0
      );
      const notesCount = playlistVideos.filter(
        (video) => video.notes && video.notes.trim()
      ).length;
...
router.get("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (playlist.userId.toString() !== req.user.id) {
      return res
        .status(403)
        .json({ msg: "Not authorized to access this playlist" });
    }

    // Fetch all videos for this playlist
    const videos = await Video.find({ playlistId })
      .select(
        "title ytId status timeSpent notes aiSummary aiSummaryGenerated thumbnail duration viewCount likeCount publishedAt channelTitle description tags position resources"
      )
    ...
    const uncompletedVideos = videos.filter(
      (video) => video.status !== "completed"
    );
    const averageVideoDuration =
      totalVideos > 0 ? totalDurationMinutes / totalVideos : 0;
    const estimatedTimeLeft = Math.round(
      averageVideoDuration * uncompletedVideos.length
    );
    ...
    const formattedResponse = {
      ...
      progress: Math.round(progress),
      completed: playlist.completed,
      totalTimeSpent: videos.reduce(
        (sum, video) => sum + (video.timeSpent || 0),
        0
      ),
      notesCount: videos.filter((video) => video.notes && video.notes.trim())
        .length,
      createdAt: playlist.createdAt,
...
router.post("/:id/add-video", authenticateToken, async (req, res) => {
  try {
    ...
    if (!playlist.isCustomPlaylist) {
      return res
        .status(403)
        .json({ msg: "Videos can only be added to custom playlists" });
    }
    ...
    if (existingVideo) {
      return res
        .status(400)
        .json({ msg: "Video already exists in this playlist" });
    }
...
router.post("/:id/sync", authenticateToken, async (req, res) => {
  try {
    ...
    const newVideos = ytData.videos.filter(
      (ytVideo) => !existingYtIds.includes(ytVideo.ytId)
    );
...
// Reset playlist progress (set all videos to to-watch)
router.post("/:id/reset", authenticateToken, async (req, res) => {
  try {
    ...
    await Video.updateMany({ playlistId }, { $set: { status: "to-watch" } });

    // Get updated videos
    const updatedVideos = await Video.find({ playlistId }).select(
      "title status"
    );
...
// Toggle pin status for a video
router.patch(
  "/:playlistId/toggle-pin-video/:videoId",
  authenticateToken,
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        pinned: video.pinned,
        message: video.pinned
          ? "Video pinned successfully"
          : "Video unpinned successfully",
      });
    } catch (err) {
