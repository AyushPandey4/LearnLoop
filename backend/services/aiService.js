...
// Generate content using OpenRouter API (sarvamai/sarvam-m:free)
const generateWithSarvam = async (prompt) => {
  if (!process.env.OPENROUTER_API_KEY) {
    console.error("OpenRouter API key is missing in environment variables");
    throw new SummaryGenerationError("OpenRouter API key is not configured");
  }

  try {
    console.log("Making request to OpenRouter API (sarvamai/sarvam-m:free)...");
    ...
    if (!response.data) {
      console.error("No data received from OpenRouter API");
      throw new SummaryGenerationError("No response data from OpenRouter API");
    }
    ...
    if (error.response) {
      ...
      console.error("OpenRouter API error details:", errorMessage);
      throw new SummaryGenerationError(`OpenRouter API error: ${errorMessage}`);
    }
    throw new SummaryGenerationError(
      "Failed to generate content with OpenRouter",
      error
    );
  }
};
...
// Generate a summary from transcript text using OpenRouter
const generateSummaryFromTranscript = async (transcript, videoTitle) => {
  ...
  try {
    ...
    const maxLength = 30000; // Adjust based on model limits
    ...
    const prompt = `Please provide a comprehensive summary of this educational video titled "${
      videoTitle || "Untitled"
    ...
    const summary = await generateWithSarvam(prompt);
    ...
    throw new SummaryGenerationError("Failed to generate summary", error);
  }
};
